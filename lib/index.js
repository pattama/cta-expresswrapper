 ...
 * @property {Map<Map>} routes - A Map of Maps of all the routes that have been applied in the Express App.
 * 1st level key is the HTTP method, 2nd level key is the path, value is the handler method.
 *
 */
class ExpressWrapper {
  ...
  _wrap(method) {
    ...
    that[method] = function(path, callback) {
      if (that.routes.has(method) && that.routes.get(method).has(path)) {
